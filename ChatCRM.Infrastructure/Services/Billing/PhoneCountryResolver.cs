...
{
    /// <summary>
    /// Best-effort country (ISO alpha-2) inference from an E.164-style WhatsApp phone number.
    /// Pricing rules are versioned per country, so this only needs to be "good enough to pick
    /// the right rule" — when no prefix matches we hand "**" to the pricing engine and it falls
    /// back to the default rule.
    ///
    /// Order matters in the lookup: longer prefixes are checked first so that "1242" (Bahamas)
    /// wins over "1" (US/Canada).
    /// </summary>
    public static class PhoneCountryResolver
    {
        private static readonly (string Prefix, string Alpha2)[] PrefixMap =
        {
            // 4-digit
        ...
        /// <summary>Strip non-digit chars and look up the longest matching ITU prefix.</summary>
        public static string? Resolve(string? phoneNumber)
        {
            ...
            foreach (var c in phoneNumber)
                if (c >= '0' && c <= '9') digits[len++] = c;

            if (len == 0) return null;
            var msisdn = new string(digits[..len]);

            foreach (var (prefix, alpha2) in PrefixMap)
            {
                if (msisdn.StartsWith(prefix, StringComparison.Ordinal))
                    return alpha2;
            }
            return null;
        }
